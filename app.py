...
def ready():
    try:
        if container and container.read_item("dummy", partition_key="dummy"):  # Test Cosmos DB connection
            return "Ready", 200
...
# trigger API URL
TRIGGER_URL = os.environ.get("TRIGGER_API_URL", "http://trigger")                                 # this value will be injected into trigger container - find this in main.tf

def trigger_user():
    try:
        requests.post(f"{TRIGGER_URL}/trigger", timeout=2)                                         # performs HTTP POST to the path /trigger
    except Exception as e:  
        print(f"Notification failed: {e}")                                                        # will catch and print exception



# Sentiment Analysis API URL (container app DNS or internal DNS)
SENTIMENT_API_URL = os.environ.get("SENTIMENT_API_URL", "http://localhost:80/analyze")

# Function to call the sentiment analysis service
def analyze_message_sentiment(message):
    print(f"[DEBUG] SENTIMENT_API_URL = {SENTIMENT_API_URL}")
                                                                 
    try:

        print(f"[DEBUG] Sending to Sentiment API with message: {message}")
        response = requests.post(                                                                   # sends http post request to sentiment container - this is how the message board container calls the sentiment container
            SENTIMENT_API_URL,
            json={"message": message},                                                              # send only the message
            timeout=3
        )
        if response.status_code == 200:
            result = response.json()                                                                # converts json repsonse into python dcitionary
            return result.get("sentiment", "N/A")                                                   # extracts sentiment, if not available it will fallback to n/a
        else:
            print(f"Sentiment API error: {response.status_code}")                                   # if response was not 200, it will print the error code and return n/a
            return "N/A"
    except Exception as e:
...
def index():
    if request.method == 'POST':                                                                    # this line will run only if a form is submitted
        ...
        print(f"[DEBUG] Sentiment received: {sentiment}")
    ...
    if container:                                                                                   # runs this block only if connected to cosmos and contatiner is available
        try:
            ...
            messages = [{"id": item["id"], "message": item["message"], 'sentiment': item.get("sentiment", "N/A")} for item in items]    # for each dictionary/'item' in the list 'items', pull out the ID and message, put them in a new dictionary and add that dict to the 'messages' list
    ...
    return render_template('index.html', messages=messages, visits=visits)                          # passes 'messages' and 'visits' to index.html, uses jinja2 to syntax
