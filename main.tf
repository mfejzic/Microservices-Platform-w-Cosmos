...
Comment out the client.messages.create() line in trigger.py temporarily and replace it with a dummy response (e.g., return jsonify({"status": "test"}), 200).
Redeploy the trigger-app image to ACR and reapply Terraform.
If the revision starts, the Twilio authentication is likely the blocker.
...
### learn attributes, command, probes etc in container app block ###
...
resource "azurerm_container_app" "main" {
  ...
  template {
    ...
    container {                                                                   // single container instance will bootup after init_container exits
      ...
      #   liveness_probe {                                                   // determines if container is alive or should continue running
      #   port = 80
      #   transport = "HTTP"
      #   path = "/health"                                                 // expects 200-399 response
      #   initial_delay = 10
      #   interval_seconds = 15
      ...
      # confirms connection w/ cosmos before serving traffic
      readiness_probe {                                                   // determines if container is ready to serve traffic
        port = 80
        transport = "HTTP"
        path = "/ready"                                                  // endpoints to flask app to return 200 when ready
        initial_delay = 10
        interval_seconds = 15
        failure_count_threshold = 3
        timeout = 5
...
resource "azurerm_container_app" "sentiment" {
  ...
  depends_on = [ azurerm_container_registry.acr, null_resource.push_image ]
}
...
resource "azurerm_container_app" "prometheus" {
  ...
  template {
    ...
    container {
      name  = "prometheus"
      image = "mf37registry.azurecr.io/prom-app:latest"             # Use your pushed image
      cpu    = 0.5
      memory = "1Gi"
...
resource "azurerm_container_app" "grafana" {
  ...
  ingress {
    external_enabled = true  # Expose Grafana UI, adjust if internal only
    target_port      = 3000
  ...
  template {
    ...
    volume {                                                                                     // declares source of storage
      name = "data"
      //storage_name = 
      storage_type =  "EmptyDir"                                                                  // memory persists for containers lifetime but is lost upon restart
    }
    
    container {
      name  = "grafana"
      image = "mf37registry.azurecr.io/grafana-app:latest"  # Use your pushed image
      cpu    = 0.5
      memory = "1Gi"
      ...
      volume_mounts {                                                                            // maps storage in containers filesystem
        name = "data"
...
// this will push all images into acr before creating the container app - container app will pull image from acr after

resource "null_resource" "push_image" {                                                             //refers to ps1 script to push image
...
#     - push prometheus -     #
  provisioner "local-exec" {
    command = "powershell.exe -ExecutionPolicy Bypass -File ./monitoring/prom-push.ps1"
  }

#     - push grafana -     #
  provisioner "local-exec" {
    command = "powershell.exe -ExecutionPolicy Bypass -File ./monitoring/grafana-push.ps1"
  }
